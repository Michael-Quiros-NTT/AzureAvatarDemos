...
function connectAvatar() {
    ...
    // Update status bar to show connecting state
...
function appendChatBubble(roleClass, contentHtml) {
...
function ensureAssistantBubble() {
    if (!activeAssistantBubble) {
        activeAssistantBubble = appendChatBubble('chat-bubble--assistant', '')
    }
...
function speakNext(text, endingSilenceMs = 0, skipUpdatingChatHistory = false) {
    ...
    if (enableDisplayTextAlignmentWithSpeech && !skipUpdatingChatHistory) {
        ...
        const assistantBubble = ensureAssistantBubble()
        assistantBubble.innerHTML += renderedText
...
function handleUserQuery(userQuery, userQueryHTML, imgUrlPath) {
    ...
    const userContentHtml = imgUrlPath.trim()
        ? userQueryHTML
        : htmlEncode(userQuery).replace(/\n/g, '<br/>')
    appendChatBubble('chat-bubble--user', userContentHtml)
    ...
    let url = "{AOAIEndpoint}/openai/deployments/{AOAIDeployment}/chat/completions?api-version=2024-02-01".replace("{AOAIEndpoint}", azureOpenAIEndpoint).replace("{AOAIDeployment}", azureOpenAIDeploymentName)
    ...
    if (dataSources.length > 0) {
        url = "{AOAIEndpoint}/openai/deployments/{AOAIDeployment}/extensions/chat/completions?api-version=2024-02-01".replace("{AOAIEndpoint}", azureOpenAIEndpoint).replace("{AOAIDeployment}", azureOpenAIDeploymentName)
        body = JSON.stringify({
    ...
    .then(response => {
        ...
        let chatHistoryTextArea = document.getElementById('chatHistory')
        const assistantBubble = appendChatBubble('chat-bubble--assistant', '')
        ...
        function read(previousChunkString = '') {
            return reader.read().then(({ value, done }) => {
                ...
                if (!enableDisplayTextAlignmentWithSpeech) {
                    assistantBubble.innerHTML += displaySentence.replace(/\n/g, '<br/>')
                    chatHistoryTextArea.scrollTop = chatHistoryTextArea.scrollHeight
                    displaySentence = ''
                }
    ...
    .catch((error) => {
        activeAssistantBubble = null
        console.log(`Error in handleUserQuery stream: ${error}`)
    })
}
...
// Función pública de toggle
function togglePreviewMode() {
  if (sessionActive) {
    alert('No puedes activar Preview Mode mientras hay una sesión activa. Cierra la sesión primero.');
    return;
  }
...
function activatePreviewMode() {
  ...
  // Mostrar videoContainer
  var videoContainer = document.getElementById('videoContainer');
  videoContainer.hidden = false;
  
  // Mostrar overlayArea
  var overlayArea = document.getElementById('overlayArea');
  overlayArea.hidden = false;
  
  // Inyectar placeholder de avatar
  var remoteVideo = document.getElementById('remoteVideo');
  remoteVideo.innerHTML = '<div class="avatar-placeholder"><span>Avatar Preview</span></div>';
  
  // Inyectar mensajes de muestra
  var chatHistory = document.getElementById('chatHistory');
  chatHistory.hidden = false;
  chatHistory.innerHTML = '<div class="chat-bubble chat-bubble--user">Hola, ¿cómo estás?</div>' +
    '<div class="chat-bubble chat-bubble--assistant">¡Hola! Soy tu asistente virtual. ¿En qué puedo ayudarte hoy?</div>' +
    '<div class="chat-bubble chat-bubble--user">¿Qué puedes hacer?</div>' +
    '<div class="chat-bubble chat-bubble--assistant">Puedo responder preguntas, ayudarte con información y mantener una conversación natural.</div>';
  
  // Mostrar subtitles con texto de muestra
  var subtitles = document.getElementById('subtitles');
  subtitles.hidden = false;
  subtitles.textContent = 'Subtitle preview text...';
  
  // Habilitar tipo de mensaje
  var showTypeMessage = document.getElementById('showTypeMessage');
  showTypeMessage.disabled = false;
  showTypeMessage.checked = true;
  document.getElementById('userMessageBox').hidden = false;
  
  // Mostrar preview banner
...
function deactivatePreviewMode() {
  ...
  // Deshabilitar tipo de mensaje
  var showTypeMessage = document.getElementById('showTypeMessage');
  showTypeMessage.disabled = true;
  showTypeMessage.checked = false;
  document.getElementById('userMessageBox').hidden = true;
  
  // Ocultar preview banner
  var previewBanner = document.getElementById('previewBanner');
  if (previewBanner) {
...
function updateStatusBar(status) {
  var statusDot = document.getElementById('sessionStatus');
  var statusLabel = document.getElementById('sessionStatusLabel');
  ...
  switch(status) {
    ...
    case 'preview':
      statusDot.className = 'status-dot status-dot--connecting';
      statusLabel.textContent = 'Preview Mode';
      break;
    case 'disconnected':
    default:
...
// Hook en los puntos críticos de sesión
// Buscar en chat.js las funciones: startSession, stopSession, connectAvatar, handleConnectionStateChange
// Insertar updateStatusBar() llamadas en los puntos correctos

// Por ahora, dejar placeholder para que integrador manual pueda buscar
// Las llamadas necesarias son:
// - connectAvatar() → llamar updateStatusBar('connecting') al inicio
// - En event de WebRTC connection established → updateStatusBar('connected')
// - En stopSession() o desconexión → updateStatusBar('disconnected')
